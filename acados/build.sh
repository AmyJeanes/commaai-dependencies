...
if [ -n "$WINDOWS" ]; then
  uv pip install --python casadi-venv/Scripts/python.exe --no-deps --quiet "casadi==$CASADI_VERSION"
  mv casadi-venv/Lib/site-packages/casadi "$CASADI_DIR"
else
...
rm -rf !(__init__.py|casadi.py|_casadi.so|_casadi.pyd|tools|libcasadi.*|libc++.*|*.dll)
shopt -u extglob
